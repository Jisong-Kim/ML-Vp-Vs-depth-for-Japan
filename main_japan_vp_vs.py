...
##### Load model #####
######################
models = pickle.load(open(r"C:\Users\JISONG\Documents\JISONG_STORAGE_C\pr4_DL_GeoInfo\0.Code\software/models.dat", "rb")) # model load
encoders = pickle.load(open(r"C:\Users\JISONG\Documents\JISONG_STORAGE_C\pr4_DL_GeoInfo\0.Code\software/encoders.dat", "rb"))
...
while True:
    ...
    # KiK-net
        ...
        df_sr_vp = encoders['soil_rock_type_kiknet_vp'].transform(df['soil_rock_type'])
        df_geo_vp = encoders['geology_kiknet_vp'].transform(df['geology'])
        df_sr_vs = encoders['soil_rock_type_kiknet_vs'].transform(df['soil_rock_type'])
        df_geo_vs = encoders['geology_kiknet_vs'].transform(df['geology'])
        idx = ['latitude', 'longitude', 'slope', 'elevation', 'geology_1',
               'geology_2', 'geology_3', 'geology_4', 'geology_5',
               'geology_6', 'geology_7', 'soil_rock_type_1', 'soil_rock_type_2', 
        ...
        df_vp = pd.concat([df, df_sr_vp, df_geo_vp], axis=1)[idx]
        df_vs = pd.concat([df, df_sr_vs, df_geo_vs], axis=1)[idx]
        window['-FB3-'].update('Success')
        
    elif event == '-FB4-':
        res_vp = models['kiknet_vp'].predict(df_vp)
        res_vs = models['kiknet_vs'].predict(df_vs)
